...
#' @export
starryeyes <- function(username, password) {

    # loadNamespace("shiny")
    # loadNamespace("DT")
    # loadNamespace("shinydashboard")

    my_feed <- obtain_feed(username, password)
    my_table <- extract_feed(my_feed)
    ...
    ui <- shinydashboard::dashboardPage(
        shinydashboard::dashboardHeader(title = my_feed$user),
